...
import {ArrowRightOnRectangleIcon, LockClosedIcon, MagnifyingGlassIcon, PlusIcon} from '@heroicons/vue/24/outline'
      ...
      <div class='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4'>
        ...
        <div class='flex-1 max-w-md relative'>
          <input
              ...
              class='w-full pl-10 pr-4 py-2 rounded-full bg-gray-100 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition'
          >
        ...
        <div class='flex items-center gap-2 shrink-0'>
          <template v-if='store.isLoggedIn'>
            <button @click="router.push('/pridat')" class='p-2 text-blue-600 hover:bg-blue-50 rounded-full'
                    title='Přidat recept'>
              <PlusIcon class='w-6 h-6'/>
            </button>
            <button @click='handleLogout' class='p-2 text-red-500 hover:bg-red-50 rounded-full' title='Odhlásit'>
              <ArrowRightOnRectangleIcon class='w-6 h-6'/>
            </button>
          </template>
          <template v-else>
            <button @click='showLoginModal = true' class='text-sm font-medium text-gray-500 hover:text-blue-600'>
              Spravovat
            </button>
          </template>
        </div>
    ...
    <div v-if='showLoginModal' class='fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4'>
      <div class='bg-white p-6 rounded-lg shadow-xl w-full max-w-xs'>
        ...
        <div class='flex justify-end gap-2'>
          <button @click='showLoginModal = false' class='px-4 py-2 text-gray-500'>Zrušit</button>
          <button @click='handleLogin' class='px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700'>Vstoupit
          </button>
        </div>
      </div>
    </div>
